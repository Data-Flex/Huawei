...
void solve_one_string (double* a, double* b, double* c, int* rootcount, double* x1, double* x2, FILE *fsquare, int* check_not_EOF);
...
int main(void) {
    ...
    if (*gets(filename) == NULL) printf ("Wrong name of file\n");
        ...
        solve_one_string (&a, &b, &c, &rootcount, &x1, &x2, fsquare, &check_not_EOF);
